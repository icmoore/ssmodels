kalmanLL=function(y,mod){
  ...
  for(i in 1:N-1){  
    ...
    innov     = as.matrix(as.numeric(y[i+1,1] - yhat_t1t));
    a_t1t1 = a_t1t + (delta_t1 %*% innov);
    P_t1t1 = P_t1t - (delta_t1 %*% t(Z) %*%  P_t1t);
    
    if(omega <= 0) omega = 0.000001
  ...
  out = list(LL = L, LLd = Ld, sigmahat2 = sigmahat2) 
